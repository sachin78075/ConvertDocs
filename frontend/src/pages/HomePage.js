...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {showConversionModal && fileInfo && (
        <div 
          className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4"
          onClick={() => setShowConversionModal(false)}
          data-testid="conversion-modal-overlay"
        >
          <motion.div
            initial={{ opacity: 0, scale: 0.95 }}
            animate={{ opacity: 1, scale: 1 }}
          ...
          >
            {/* Modal Header */}
            <div className="sticky top-0 bg-gradient-to-r from-red-600 to-yellow-500 p-6 rounded-t-3xl">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-bold text-white">Choose Conversion Type</h2>
                <button
                  onClick={() => setShowConversionModal(false)}
            ...
            <div className="p-6 border-b border-gray-200">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{fileInfo.type} • {fileInfo.size} MB</p>
                </div>
              </div>
            </div>
            ...
            {/* Conversion Options */}
            <div className="p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Select conversion format:</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {fileInfo.options.map((option, index) => (
                  <motion.button
                    key={index}
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.2, delay: index * 0.05 }}
                    onClick={() => handleConversionChoice(option.route)}
                    className="flex items-center gap-4 p-4 rounded-2xl border-2 border-gray-200 hover:border-yellow-500 hover:shadow-lg transition-all duration-300 text-left group"
                    data-testid={`conversion-option-${index}`}
                  >
                    ...
                    <svg className="w-5 h-5 text-gray-400 group-hover:text-red-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
